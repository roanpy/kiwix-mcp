...
- `search`：在选定存档中检索，精确标题优先，支持分页、估算命中数和每条结果的
  ...
  若两个存档覆盖相同内容，同一 `article_path` 可能每个存档各出现一次——这是刻意的，
  因为它们是不同快照。只搜单个存档或按 `flavour` 过滤即可避免重复。
...
- **只读。** 没有任何工具会写入存档。`extract_image` 是唯一在读取之外触及文件系统的工具：
  它把解码后的图片缓存在 `~/.cache/kiwix-mcp/images` 下，去重并最多保留 16 个文件。可用
  `KIWIX_MCP_IMAGE_DIR` 改变该位置。这个路径刻意选在稳定位置而非临时目录，因为返回的
  `file_path` 会交给客户端在调用结束后再读取。
...
`note` 说明原因，以免把空结果误判为存档损坏。这些条目仍可通过标题检索发现。
